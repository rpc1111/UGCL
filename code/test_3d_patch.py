...
# from LA_vis import visualize_segmentation
...
def var_all_case_LA(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4):
    ...
    image_list = ["../data/LA/data/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
...
def test_all_case(model, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=0, nms=0):
    ...
    for image_path in loader:
        ...
        #visualize_segmentation(image,label,"111.png")
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                with torch.no_grad():
                    ...
                    #z=y.argmax(1).reshape(112,112,80)
                # y_sdf, y = model.inference(test_patch)
                y = y.cpu().data.numpy()
                #visualize_segmentation(test_patch.reshape(112,112,80).cpu().data.numpy(),z.cpu().data.numpy(),"222.png")
                y = y[0,:,:,:,:]
