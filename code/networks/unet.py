...
class Decoder(nn.Module):
    def __init__(self, params):
       ...
       # self.selfattention=selfattention(self.ft_chns[0])
        self.projector = nn.Sequential(
            nn.Conv2d(self.ft_chns[0], self.ft_chns[0], kernel_size=1, stride=1),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(self.ft_chns[0],3
                      * self.ft_chns[0], kernel_size=1, stride=1),
        )
    ...
    def forward(self, feature):
        ...
        #     dims = torch.tensor(x.size(1) * x.size(2)*x.size(3))
        #     mag_norm = 10.0/torch.sqrt(dims)
        #     x1=x + torch.zeros_like(x).uniform_(-mag_norm, mag_norm)
        # else:
        #     x1=x
        # #print(x[2,:,2,2])
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        output,proj = self.decoder(feature)
        return output,proj
